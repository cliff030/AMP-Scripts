...
function EmailReport($ReportName,$ReportFile,$ReportDate) {
   if($ReportName -eq "ConvertedLeadsByDateRange" ) {
    ...
    } elseif($ReportName -eq "ConvertedLeadsByDateRangeSummary" ) {
        $ReportName = "Converted Leads Summary"
	}
    
    $ReportName = $global:Company."Name" + " $ReportName"

    $Sender = New-Object Net.Mail.MailAddress("reports@ampaccount.com","AMP Reports")
    $Recipient = New-Object Net.Mail.MailAddress("nruggeri@aol.com","Nick Ruggeri")
...
$Reports = ("ConvertedLeadsByDateRange", "NewLeadsByDateRange", "ConvertedLeadsByDateRangeSummary")
